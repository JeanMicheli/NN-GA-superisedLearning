...
class CrossEntropyCost(object):
    ...
    def fn(a, y):
        ...
        """
        temp = np.sum(np.nan_to_num(-y*np.nan_to_num(np.log(a))-(1-y)*np.nan_to_num(np.log(1-a))))
        if  np.isnan(temp):
...
class Network(object):
    ...
    def __init__(self, connections, cost=CrossEntropyCost):
        ...
        self.sizes = [len(connections[i]) for i in range(self.num_layers)]
    ...
    def default_weight_initializer(self):
        ...
        for layer in range(1,len(self.connections)):  # for each layer (except input layer)
            self.weights[layer-1] = [0]*len(self.connections[layer])
            self.mask_matrix[layer-1] = [0]*len(self.connections[layer])
            for neuron in range(len(self.connections[layer])):  # for each neuron of current layer
                nr_connections = len(self.connections[layer][neuron])
                # initialise all weights as it would be fully connected
                self.weights[layer-1][neuron] = [0] * len(self.connections[layer-1])
                self.mask_matrix[layer - 1][neuron] = [0] * len(self.connections[layer-1])
                for active_con in self.connections[layer][neuron]:
                    # set radnom value only for weights actually entering in the neuron
                    self.weights[layer-1][neuron][active_con] = np.random.randn() * np.sqrt(nr_connections)
                    self.mask_matrix[layer-1][neuron][active_con] = 1
            self.weights[layer - 1] = np.array(self.weights[layer-1])
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta,
        ...
        if evaluation_data:
            evaluation_data = list(copy.deepcopy(evaluation_data))
